...
def save(event=None):
	...
	if(len(menu) == 0):
		...
		try:
			...
			writer=csv.writer(f)
			writer.writerow([time,menu,cost,amount,total])
			f.close()

			#Update data in tree view
			tree.insert(parent='',index='end',value=(time,menu,cost,amount,total))
...
#Tab2
#Making tree view
header = ['Date','Menu','Cost','Amount','Total']
width = [190,170,90,90,90]
